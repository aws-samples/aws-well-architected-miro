...
aws lambda update-function-code --function-name AnswersListFunction --image-uri "${account}.dkr.ecr.${region}.amazonaws.com/getanswerslist:latest"
aws lambda update-function-code --function-name WorkloadListFunction --image-uri "${account}.dkr.ecr.${region}.amazonaws.com/getworkloadlist:latest"
aws lambda update-function-code --function-name APIGWauthFunction --image-uri "${account}.dkr.ecr.${region}.amazonaws.com/authorize:latest"
aws lambda update-function-code --function-name UserOnboardFunction --image-uri "${account}.dkr.ecr.${region}.amazonaws.com/onboard:latest"
aws lambda update-function-code --function-name WorkloadFunction --image-uri "${account}.dkr.ecr.${region}.amazonaws.com/getworkload:latest"